...
class Jarvis:
    def __init__(self, data_filepath='data.parquet', emotions_data_path='emotions_dataset_kaggle.csv'):
        ...
        self.weather_api_key = os.getenv('a6f25d9e92bf342a9033662c09016522')
        ...
        #self.check_api_keys()
        self.engine = pyttsx3.init()  # Initialize Text-to-Speech engine
        self.analyzer = BERTAnalyzer(data_path=emotions_data_path)
        dataset = pd.read_csv('emotions_dataset_kaggle.csv')
        encodings, labels = self.analyzer.preprocess(dataset)
        self.analyzer.train(encodings, labels, epochs=3)
    ...
    def predict_intent(self, input_text):
        predicted_intent = self.analyzer.predict(input_text)
...
if __name__ == '__main__':
    jarvis = Jarvis(emotions_data_path='emotions_dataset_kaggle.csv')

    analyzer = jarvis.analyzer  # Get the BERTAnalyzer object

    # Read the dataset and preprocess it:
    dataset = pd.read_csv('emotions_dataset_kaggle.csv')
    input_ids, attention_masks, labels = analyzer.preprocess(dataset)
    ...
    analyzer.train(train_inputs, train_masks, train_labels, epochs=3)

    # Try to predict using the trained model:
    print(analyzer.predict("Sample input text"))

    # Now that everything has been tested, start the main assistant's loop
    jarvis.main_loop()